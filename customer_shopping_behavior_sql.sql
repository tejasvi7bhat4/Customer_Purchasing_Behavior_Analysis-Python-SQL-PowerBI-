...
SELECT gender, SUM(purchase_amount) AS revenue FROM customer_behavior.customer
GROUP BY gender;
...
Select shipping_type, avg(purchase_amount) as "Average_Purchase_Amount" from customer where shipping_type in ('Express','Standard')
group by shipping_type;

# Q5. Do subscribed customers spend more? Compare average spend and total revenue between subscribers and non-subscribers.
select subscription_status,count(customer_id) as total_cutomers, avg(purchase_amount) as "Average_Spend",sum(purchase_amount) as "Total_Revenue" from customer 
...
from customer_behavior.customer group by item_purchased 
